function [LineOutput LinkedList LinesList] = getInput2(f, Point3D, FIXED_AXES_HANDLE, DIRECTION, LinkedList, LinesList)
...
if detect_point_or_line(Point) == 1 %% POINT
    ...
    disp(['Added a new point ', num2str(PointCounter)]);
    ...
    disp(['Added a STRAIGHT line from ', num2str(EndPoint1_2D_index), ' to ', num2str(EndPoint2_2D_index)]);
    
    %% Update Linked List
    NewLink = [EndPoint1_2D_index EndPoint2_2D_index];
    
    if LinkedList == 0
        LinkedList = NewLink;
    else
        LinkedList = vertcat(LinkedList, NewLink);
    end;
    ...
    if DIRECTION == 0 %% NORMAL
        ...
        %% Update Lines List
        NewLineList=[Point(1,1) Point(1,2) Point(end,1) Point(end,2)];
        if LinesList == 0
            LinesList = NewLineList;
        else
            LinesList(end+1,1:4) = NewLineList;
        end;
    ...
    elseif DIRECTION == 1 %% UP and DOWN        
        ...
        %% Update Lines List
        NewLineList=[PointA(1), PointA(2), PointC(1), PointC(2), ...
                     PointD(1), PointD(2), PointB(1), PointB(2)];
                 
        if LinesList == 0
            LinesList = NewLineList;
        else
            LinesList(end+1,1:8) = NewLineList;
        end;
    ...
    elseif DIRECTION == 3 %% Down(Left)...
        ...
        %% Update Lines List
        NewLineList=[PointA(1), PointA(2), PointC(1), PointC(2), ...
                     PointD(1), PointD(2), PointE(1), PointE(2), ...
                     PointF(1), PointF(2), PointB(1), PointB(2), ...
                     ];
                 
        if LinesList == 0
            LinesList = NewLineList;
        else
            LinesList(end+1,1:12) = NewLineList;
        end;
    ...
    [IntPoint LinkedList LinesList] = CheckIntersections(LinkedList, LinesList, Point2D);
    
    %PointCounter    = PointCounter+1;
    if IntPoint ~=0
        PointCounter = evalin('base','PointCounter');
        for i=1:numel(IntPoint(:,1))
             PointCounter    = PointCounter+1;
             Point2D(PointCounter,1:2) = [IntPoint(i,1) IntPoint(i,2)];
             
             plot(IntPoint(i,1), IntPoint(i,2), '.', 'Color', 'r', 'Markersize', 25);
             text(IntPoint(i,1)-0.07, IntPoint(i,2)-0.07, num2str(PointCounter), 'FontSize', 15, 'Color', 'r');   
        end;
